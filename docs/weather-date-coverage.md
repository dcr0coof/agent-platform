...
状态：Issue #4 的首个工具切片，依赖出行工作台 PR #12；天气证据卡、备选活动及行程草稿切换仍待实现。
...
本切片不判断数据是否已超过新鲜度阈值、不补齐缺失数据、不获取历史天气，也不保证模型一定遵守工具结果。城市消歧仍沿用第一条城市搜索结果，Location ID 用于核对，尚未提供候选地点确认界面。前端当前显示模型回复和运行状态，未增加独立天气卡片。
