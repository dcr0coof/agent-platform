...
切换会话等待响应期间暂停编辑、发送、新建及再次切换；目标成功加载后才替换原会话并切换事件订阅。失败时保留原会话及其运行进度，显示错误后恢复操作，避免慢请求与旧会话操作相互覆盖。

未发送消息按出行会话暂存于当前页面：切换或新建后，再返回原出行可继续编辑；加载失败和同会话保存不会覆盖输入。服务器接受发送后清空该草稿。草稿不写入服务器、浏览器存储或模型上下文，刷新/关闭页面后清除，界面在有输入时提示此限制。约束表单仍需保存，不属于消息草稿。
...
满载时也保持同一判断顺序：已存在的相同请求返回原任务（202），同一键对应不同消息返回冲突（409），需要新建任务时才返回容量不足（429）。拒绝的请求不创建任务记录；数据库读取错误不会被伪装成容量不足。

工作台每个 JSON API 请求设有 15 秒客户端超时，覆盖等待响应和读取响应体；请求结束清理定时器，并保留调用者主动取消的能力。超时恢复界面操作并保留尚未发送成功确认的消息，不会自动重放写请求。发送响应丢失后，在当前会话中保持消息不变再次发送，会沿用原请求 ID；切换会话或刷新会丢失这个临时重试 ID，应先查看已保存的运行/对话。新建或保存出行超时后也应刷新确认结果，再决定下一步操作。客户端超时不等于取消服务端任务，不影响 SSE 长连接或服务端整体运行时限。
