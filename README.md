...
工具新增可选 `date: "YYYY-MM-DD"`：指定日期时默认查询预报，只返回该日期的数据，未覆盖则明确未知。预报附带来源、Location ID、获取时间、Provider 更新时间和实际可用日期。调用与边界见 [按日期查询天气](docs/weather-date-coverage.md)。天气证据卡和活动备选尚未实现；现有 v7 预报接口的官方弃用提示与迁移依赖也记录在该文档中。
